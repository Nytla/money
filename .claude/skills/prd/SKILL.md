...
PRD — первое звено: [[prd]] → [[decompose]] → [[plan-phase]] → [[issues]].
Критерии готовности из этого документа станут каркасом таблицы покрытия при
декомпозиции, поэтому каждый критерий должен быть проверяемым утверждением,
а не намерением. Здесь только «что и зачем»; дробление на задачи и порядок фаз —
не в этом файле.
